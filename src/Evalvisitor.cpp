...
std::any EvalVisitor::visitFile_input(Python3Parser::File_inputContext *ctx) {
  ...
  // std::cout<<"!!!";
  for (int i = 0; (i < stmtlist.size() && stmtlist[i]) ||
                  (i < linelist.size() && linelist[i]);
       i++) {
...
std::any EvalVisitor::visitExpr_stmt(Python3Parser::Expr_stmtContext *ctx) {
  ...
  if (ctx->augassign())
    Assign = visitAugassign(ctx->augassign());
  int *pp = std::any_cast<int>(&Assign);
  int p = *pp;
  if (p != EQAL) {
    result = visitTestlist(testlist.front());
    std::vector<std::any> varylist =
        std::any_cast<std::vector<std::any>>(result);
    for (int j = 0; j < varylist.size() && j < val.size(); j++) {
      ...
      if (p == DIVdouble) {
        Changevalue(varylist[j],
                    Getdivdoubleright(Getvalue(varylist[j]), Getvalue(val[j])));
      }
      if (p == MOD) {
        Changevalue(varylist[j], Getvalue(varylist[j]) % Getvalue(val[j]));
      }
...
std::any
EvalVisitor::visitContinue_stmt(Python3Parser::Continue_stmtContext *ctx) {
  return Flowcontrol(Flowcontrol::controltype::continue_, {});
}
